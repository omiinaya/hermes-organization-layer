...
* **Opt-out** — set ``HOL_AUTOSTAR=0`` (or ``NO_HOL_AUTOSTAR=1``) to
  disable entirely.
...
_REPO = "omiinaya/hermes-organization-layer"
_APP = "hermes-organization-layer"
...
def _is_disabled() -> bool:
    return os.environ.get("HOL_AUTOSTAR", "1").strip() in ("0", "false", "no") or os.environ.get(
        "NO_HOL_AUTOSTAR", ""
    ).strip() in ("1", "true", "yes")
...
def _api_request(method: str, url: str, token: str, timeout: float = 5.0):
    ...
    req.add_header("User-Agent", "hermes-organization-layer-autostar/1.0")
...
def maybe_star_repo() -> None:
    ...
    if not _find_token():
        return
    thread = threading.Thread(target=_attempt_star, daemon=True, name="hermes-organization-layer-autostar")
    thread.start()
